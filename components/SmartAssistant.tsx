    ...
    try {
      ...
      const systemPrompt = `
        ...
        **RESPONSE FORMAT & ACTIONS:**
        ...
        - **Direct Actions (Function Calling):** If the user asks to "เลื่อนงาน", "อัปเดตสถานะงาน", "บันทึกเครื่องจักรเสีย", or "จำว่า...", you MUST use the provided tools (Function Calling) to execute the action immediately. Do not return a JSON block for these.
      ...
      const generateMissingBomsFunc: FunctionDeclaration = {
        name: "generateMissingBoms",
        description: "สร้างสูตรการผลิต (BOM) อัตโนมัติสำหรับสินค้าทุกรายการที่ยังไม่มีสูตร โดยอ้างอิงจากสเปคสินค้าและวัตถุดิบที่มีในคลัง",
        parameters: {
          type: Type.OBJECT,
          properties: {},
        }
      ...
      if (response.functionCalls && response.functionCalls.length > 0) {
        for (const call of response.functionCalls) {
          ...
          } else if (call.name === 'generateMissingBoms') {
            if (onGenerateMissingBoms) {
