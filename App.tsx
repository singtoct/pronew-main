...
import { generateSuggestedBOMs } from './utils/bomCalculator';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const unsubscribeJobs = onSnapshot(collection(db, 'jobs'), (snapshot) => {
      ...
      snapshot.forEach((doc) => {
        const data = doc.data();
        jobsData.push({ ...data, id: doc.id } as ProductionJob);
      });
      setJobs(jobsData);
    }, (error) => {
    ...
    const unsubscribeLogs = onSnapshot(collection(db, 'logs'), (snapshot) => {
      ...
      snapshot.forEach((doc) => {
        const data = doc.data();
        logsData.push({ ...data, id: doc.id } as AuditLog);
      });
    ...
    const unsubscribeForms = onSnapshot(collection(db, 'forms'), (snapshot) => {
      ...
      snapshot.forEach((doc) => {
        const data = doc.data();
        formsData.push({ ...data, id: doc.id } as FormTemplate);
      });
    ...
    const unsubscribeDowntimeLogs = onSnapshot(collection(db, 'downtimeLogs'), (snapshot) => {
      ...
      snapshot.forEach((doc) => {
        const data = doc.data();
        downtimeData.push({ ...data, id: doc.id } as DowntimeLog);
      });
    ...
    const unsubscribeCustomKnowledge = onSnapshot(collection(db, 'customKnowledge'), (snapshot) => {
      ...
      snapshot.forEach((doc) => {
        const data = doc.data();
        knowledgeData.push({ ...data, id: doc.id } as CustomKnowledge);
      });
    ...
    const unsubscribeInventory = onSnapshot(collection(db, 'inventory'), (snapshot) => {
      if (!snapshot.empty) {
        ...
        snapshot.forEach((doc) => {
          const data = doc.data();
          inventoryData.push({ ...data, id: doc.id });
        });
  ...
  const handleGenerateMissingBoms = async () => {
    const newBoms = generateSuggestedBOMs(PRODUCT_SPECS, inventory, boms);
    if (newBoms.length === 0) return 0;

    const batch = [];
    for (const bom of newBoms) {
      const newBomRef = doc(collection(db, 'boms'));
      batch.push(setDoc(newBomRef, { ...bom, id: newBomRef.id }));
    }
    
    try {
      await Promise.all(batch);
      addLog('CREATE', `สร้างสูตรการผลิตอัตโนมัติ ${newBoms.length} รายการ`, 'SYSTEM', []);
      return newBoms.length;
    } catch (error) {
      console.error("Error generating BOMs:", error);
      return 0;
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 flex font-kanit">
      ...
      <main className="flex-1 md:ml-64 p-4 md:p-8 pt-20 md:pt-8 overflow-y-auto h-screen relative">
        ...
        <Routes>
          ...
          <Route path="/plan" element={<ProductionPlan jobs={jobs} onEditJob={handleEditJob} onViewOrder={handleViewOrder} onImportJobs={handleImportJobs} onPrintPlan={() => navigate('/plan-print')} onOpenImportModal={() => setIsImportPlanModalOpen(true)} />} />
          ...
          <Route path="/list" element={<JobTable jobs={jobs} onEditJob={handleEditJob} onPrintHandover={handlePrintHandover} onPrintTag={handlePrintTag} />} />
